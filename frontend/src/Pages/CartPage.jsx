      ...
      {checkoutRecs.length > 0 && (
        <div className="cart-recommendations-container">
          <div className="cart-rec-header">
            ...
            {/* Real-time Session Heuristic Craving Controls */}
            <div className="cart-rec-heuristics" style={{ display: "flex", flexWrap: "wrap", gap: "6px", alignItems: "center" }}>
              <button
                type="button"
                className={`heuristic-pill ${sessionContext?.active_affinity?.Spice >= 4 ? "active" : ""}`}
                style={{
                  fontSize: "11px",
                  padding: "4px 8px",
                  borderRadius: "12px",
                  border: sessionContext?.active_affinity?.Spice >= 4 ? "1px solid #e35205" : "1px solid #ddd",
                  background: sessionContext?.active_affinity?.Spice >= 4 ? "#fff3e0" : "#fff",
                  color: sessionContext?.active_affinity?.Spice >= 4 ? "#e35205" : "#555",
                  fontWeight: 600,
                  cursor: "pointer",
                }}
                onClick={() => {
                  if (sessionContext?.active_affinity?.Spice >= 4) {
                    setActiveScalarAffinity("Spice", 0);
                  } else {
              ...
              <button
                type="button"
                className={`heuristic-pill ${(sessionContext?.rejected_sub_roles || []).includes("thick_shake") ? "active" : ""}`}
                style={{
                  fontSize: "11px",
                  padding: "4px 8px",
                  borderRadius: "12px",
                  border: (sessionContext?.rejected_sub_roles || []).includes("thick_shake") ? "1px solid #d32f2f" : "1px solid #ddd",
                  background: (sessionContext?.rejected_sub_roles || []).includes("thick_shake") ? "#ffebee" : "#fff",
                  color: (sessionContext?.rejected_sub_roles || []).includes("thick_shake") ? "#d32f2f" : "#555",
                  fontWeight: 600,
                  cursor: "pointer",
                }}
                onClick={() => {
                  if ((sessionContext?.rejected_sub_roles || []).includes("thick_shake")) {
                    unrejectSubRole("thick_shake");
                    unrejectSubRole("hot_coffee");
                  } else {
                    rejectSubRole("thick_shake");
                    rejectSubRole("hot_coffee");
                  }
                }}
              >
                🚫 No Shakes/Coffee
              ...
              <button
                type="button"
                className={`heuristic-pill ${sessionContext?.velocity_state === "rushed" ? "active" : ""}`}
                style={{
                  fontSize: "11px",
                  padding: "4px 8px",
                  borderRadius: "12px",
                  border: sessionContext?.velocity_state === "rushed" ? "1px solid #7b1fa2" : "1px solid #ddd",
                  background: sessionContext?.velocity_state === "rushed" ? "#f3e5f5" : "#fff",
                  color: sessionContext?.velocity_state === "rushed" ? "#7b1fa2" : "#555",
                  fontWeight: 600,
                  cursor: "pointer",
                }}
                onClick={() => {
                  if (sessionContext?.velocity_state === "rushed") {
                    setVelocityState(null);
                  } else {
                    setVelocityState("rushed");
                  }
                }}
